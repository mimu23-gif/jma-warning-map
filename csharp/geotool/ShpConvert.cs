...
namespace JmaMap.Tools
{
    ...
    public static class ShpConvert
    {
        ...
        static void AppendGeometry(StringBuilder sb, ShpRecord rec)
        {
            ...
            List<List<double[]>> polygons = GroupRings(rec.Rings);
            if (polygons.Count == 1)
            {
        ...
        static void AppendPolygon(StringBuilder sb, List<double[]> rings)
        {
            sb.Append("[ ");
            for (int i = 0; i < rings.Count; i++)
            {
        ...
        static void AppendRing(StringBuilder sb, double[] ring)
        {
            sb.Append("[ ");
            int n = ring.Length / 2;
            for (int i = 0; i < n; i++)
        ...
        // OGR GeoJSONドライバと同じ書式（Cの "%.15f" 相当）で出し、末尾の 0 を落とす。
        //
        // .NET Framework の ToString("F15") は有効数字15桁で打ち切られるため使えない
        // （141.623844890429382 が 141.623844890429 になってしまう）。
        // doubleは mantissa×2^exp という厳密な有理数なので、BigInteger で 10^15 倍して
        // 丸めれば、C の printf と同じ「厳密値を小数15桁に正しく丸めた表記」が得られる。
        const int CoordDecimals = 15;
        static readonly BigInteger Scale = BigInteger.Pow(10, CoordDecimals);

        static void AppendCoord(StringBuilder sb, double d)
        {
            if (double.IsNaN(d) || double.IsInfinity(d)) { sb.Append("null"); return; }

            // 補足: GDALは "999999..." のような並びを見つけると桁を詰めた短い表記を選ぶことがあり、
            // その場合だけ文字列としては本ツールの出力と見た目が変わる（例: 144.972053060999997 と
            // 144.972053061）。どちらも読み戻せば同一のdoubleになるので数値としては等価。
            // 有効数字15桁を先に試す実装も検討したが、.NET Framework の double.Parse は
            // 正しく丸められない場合があり、往復判定を誤って別の値を書いてしまうため採用しない。
            ...
            // |d| = mantissa * 2^exponent。これを 10^15 倍して最近接偶数丸め。
