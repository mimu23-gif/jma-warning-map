...
そのため **許容誤差 0.0002度（約22m）でDouglas-Peucker簡略化した版**（126MB・292万頂点）を同梱し、
...
（[csharp/README.md](csharp/README.md#geotoolexe境界データ変換ツール) 参照）。
...
`getActiveWarningFeatures()` は、**絞り込みをかける前の全国の警報**から「いま発表されている現象コードと
...
サーバ側で絞り込み前の集合を作っています。C#移植版も同じ仕様です。
