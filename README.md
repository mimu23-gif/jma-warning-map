...
（4フォルダ・合計約14MB）。本アプリが使う4区分と元データの対応:
...
1回の取得が526MBになってブラウザが実用に耐えません。そのため
**許容誤差 0.0023度（約256m）でDouglas-Peucker簡略化した版**（14.4MB・32.4万頂点、**99.3%削減**）
を同梱しています。

| | 頂点 | データ量 | 全国分の応答 | ブラウザのJSON.parse |
|---|---|---|---|---|
| 原データそのまま | 4,502万 | 1.88 GB | 526 MB | 約5秒・ヒープ約2.5GB |
| **同梱の簡略化版** | **32.4万** | **14.4 MB** | **6.0 MB** | **55ミリ秒・ヒープ37MB** |

簡略化の副作用（把握したうえで採用しています）:

- 境界線は最大約256mずれます。ズーム18（1px≒42cm）で見ると輪郭の粗さが分かります
- **小さな島や岩礁は消えます**（北海道の一次細分区域でポリゴン 9,435 → 157）。
  離島にあるPOIが「警報発令エリア内の地点」に出なくなる可能性があります

精度が必要なら、許容誤差を変えて作り直せます（[csharp/README.md](csharp/README.md#geotoolexe境界データ変換ツール) 参照）。
原データからの再生成もコマンド1つで済むため、リポジトリには軽い版だけを置いています。
